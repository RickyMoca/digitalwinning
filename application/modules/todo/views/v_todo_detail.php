    ...
    <div class="card">
        <div class="card-header header-elements-inline bg-dark">
            <h3 class="card-title">
                ...
                <a href="<?= base_url('todo/') . $page ?>" class="text-light">
                    <i class="icon-arrow-left16 icon-2x mr-2"></i></a>
                Back
            </h3>

            <button class="btn copy btn-sm-dark" id="pilih" data-clipboard-text="<?= $detail['subject_todos'] . ' - ' . $detail['reply_todos']; ?>"><i class="icon-stack icon-1x"></i> Copy</button>
        </div>
        <div id="message"></div>
        <div class="card-body bg-light">
            ...
            <div class="col-md-12">
                <?php if ($detail['user_agent'] == $this->session->userdata('id')) { ?>
                    <a href="<?= base_url(); ?>todo/flag?ids=<?= $detail['id_todos'] . '&page=' . $page ?>"> <i class="icon-bookmark<?= $detail['flag'] ?> float-right ml-2 icon-2x text-warning"></i></a>
                <?php } ?>
